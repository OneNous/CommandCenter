...
export function DesignCanvas({ children, title, subtitle, minScale, maxScale, style }) {
    ...
    <DCCtx.Provider value={api}>
      ...
      >
        {(title || subtitle) && (
          ...
          >
            {title && (
              <div style={{ fontSize: 22, fontWeight: 650, color: DC.title, letterSpacing: -0.4 }}>{title}</div>
            )}
            {subtitle && <div style={{ fontSize: 14, color: DC.subtitle, marginTop: 4 }}>{subtitle}</div>}
          </div>
        )}
...
function DCEditable({ value, onChange, style, tag: Tag = 'span', onClick }) {
  return (
    <Tag
...
function DCFocusOverlay({ entry, sectionMeta, sectionOrder }) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- overlay is keyed by focus slot; handlers use render snapshot
  }, [])
