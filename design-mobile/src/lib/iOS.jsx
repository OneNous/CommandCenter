...
export function IOSNavBar({ title = 'Title', dark = false, trailingIcon = true }) {
  ...
  return (
    ...
    >
      ...
      >
        {pillIcon(
          <svg width="12" height="20" viewBox="0 0 12 20" fill="none" style={{ marginLeft: -1 }} aria-hidden>
            <path
              d="M10 2L2 10l8 8"
              stroke={muted}
              strokeWidth="2.5"
 ...
 * When `title` is omitted, scroll content gets top inset so it clears the overlaid status bar.
 */
export function IOSDevice({ children, width = 402, height = 874, dark = false, title, keyboard = false }) {
  const showNav = title !== undefined
  const contentTopPad = showNav ? 0 : 62
  ...
  return (
    ...
    >
      ...
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        {showNav && <IOSNavBar title={title} dark={dark} />}
        <div style={{ flex: 1, overflow: 'auto', paddingTop: contentTopPad, boxSizing: 'border-box' }}>{children}</div>
...
export function IOSKeyboard({ dark = false }) {
  ...
  const icons = {
    shift: (
      <svg width="19" height="17" viewBox="0 0 19 17" aria-hidden>
        <path d="M9.5 1L1 9.5h4.5V16h8V9.5H18L9.5 1z" fill={glyph} />
      </svg>
    ),
    del: (
      <svg width="23" height="17" viewBox="0 0 23 17" aria-hidden>
        <path
