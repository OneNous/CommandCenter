// app.jsx — compose iOS + Android frames inside a Design Canvas
...
/* Per-platform shell with bottom tabs + screen routing.
   Sky props are forwarded so all instances share the same swirl tuning. */
function PhoneShell({ platform, initialTab = "dash", sky }) {
  const [tab, setTab] = useStateApp(initialTab);
  React.useEffect(() => { setTab(initialTab); }, [initialTab]);
  const screen =
    tab === "dash"     ? <DashboardScreen/>  :
    tab === "trends"   ? <ReportsScreen/>    :
    tab === "settings" ? <SettingsScreen/>   : <DashboardScreen/>;
...
function AndroidShell({ children }) {
  return (
    <div style={{
      width: 390, height: 844, borderRadius: 18, overflow: "hidden",
      background: "#05070a",
      ...
      <div style={{
        ...
        padding:"0 18px", color:"#fff", fontFamily:"Roboto, system-ui",
...
function AndroidLogin({ sky }) {
  return <AndroidShell><LoginScreen sky={sky}/></AndroidShell>;
}

/* Patch theme tokens live so screens reflect tweak changes. */
function useApplyTheme(t) {
  React.useEffect(() => {
    if (!window.CS_T) return;
    ...
    // Translucent surfaces let ArtSky read through. We keep the same hue.
...
function App() {
  ...
  // Re-render artboards on visual tweak changes (the canvas memos by children).
  ...
  const sections = [];
  if (t.showLogin) sections.push(
    <DCSection key="login" id="login" title="Login / onboarding">
      {showIOS     && <DCArtboard id="ios-login"     label="iOS · Sign in"     width={390} height={844}><IOSLogin     sky={sky}/></DCArtboard>}
      {showAndroid && <DCArtboard id="android-login" label="Android · Sign in" width={390} height={844}><AndroidLogin sky={sky}/></DCArtboard>}
    </DCSection>
  );
  if (t.showDash) sections.push(
    <DCSection key="dash" id="dash" title="Dashboard / live">
      {showIOS     && <DCArtboard id="ios-dash"     label="iOS · Live"     width={390} height={844}><IOSPhone     tab="dash" sky={sky}/></DCArtboard>}
      {showAndroid && <DCArtboard id="android-dash" label="Android · Live" width={390} height={844}><AndroidPhone tab="dash" sky={sky}/></DCArtboard>}
    </DCSection>
  );
  if (t.showTrends) sections.push(
    <DCSection key="trends" id="trends" title="Reports / trends">
      {showIOS     && <DCArtboard id="ios-trends"     label="iOS · Trends"     width={390} height={844}><IOSPhone     tab="trends" sky={sky}/></DCArtboard>}
      {showAndroid && <DCArtboard id="android-trends" label="Android · Trends" width={390} height={844}><AndroidPhone tab="trends" sky={sky}/></DCArtboard>}
    </DCSection>
  );
  if (t.showSettings) sections.push(
    <DCSection key="settings" id="settings" title="Settings / profile">
      {showIOS     && <DCArtboard id="ios-settings"     label="iOS · Settings"     width={390} height={844}><IOSPhone     tab="settings" sky={sky}/></DCArtboard>}
      {showAndroid && <DCArtboard id="android-settings" label="Android · Settings" width={390} height={844}><AndroidPhone tab="settings" sky={sky}/></DCArtboard>}
    </DCSection>
  );
    ...
    <>
      ...
      <TweaksPanel title="Tweaks">
        <TweakSection label="Layout">
          <TweakRadio label="Platforms" value={t.platforms}
            options={[{value:"both",label:"Both"},{value:"ios",label:"iOS"},{value:"android",label:"Android"}]}
        ...
        <TweakSection label="Theme">
          ...
          <TweakSlider label="Card opacity" value={t.cardOpacity} min={0} max={100} step={5} unit="%"
            onChange={(v) => setT("cardOpacity", v)}/>
        </TweakSection>

        <TweakSection label="ArtSky · v77 fluid sky">
          <TweakSelect label="Palette" value={t.skyTheme}
            options={[
          ...
          <TweakSlider label="Speed"          value={t.skySpeed} min={0} max={200} step={2}  unit="%"
            onChange={(v) => setT("skySpeed", v)}/>
          <TweakSlider label="Zoom"           value={t.skyZoom}  min={10} max={100} step={1} unit="%"
            onChange={(v) => setT("skyZoom", v)}/>
          <TweakSlider label="Warp"           value={t.skyWarp}  min={0} max={600} step={5}
            onChange={(v) => setT("skyWarp", v)}/>
          <TweakSlider label="Wind speed"     value={t.skyWind}  min={0} max={400} step={2}
            onChange={(v) => setT("skyWind", v)}/>
          <TweakSlider label="FBM strength"   value={t.skyFbm}   min={0} max={150} step={1} unit="%"
            onChange={(v) => setT("skyFbm", v)}/>
          <TweakSlider label="Wave blur"      value={t.skyBlur}  min={0} max={300} step={2}
            onChange={(v) => setT("skyBlur", v)}/>
          <TweakSlider label="Grain"          value={t.skyGrain} min={0} max={50} step={1}  unit="‰"
            onChange={(v) => setT("skyGrain", v)}/>
          <TweakSlider label="Black veil"     value={t.blackVeil} min={0} max={90} step={5} unit="%"
            onChange={(v) => setT("blackVeil", v)}/>
        </TweakSection>

        <TweakSection label="Data">
          <TweakSlider label="Channels" value={t.channelCount} min={2} max={5} step={1}
            onChange={(v) => setT("channelCount", v)}/>
        </TweakSection>
      </TweaksPanel>
    </>
  );
