// ArtSky — Three.js port of v77's production fluid-sky shader (artsky-core.ts).
// Renders cerulean (homepage default) by default. Each instance owns its own
// canvas + animation loop. Designed to fill its parent's box.
//
// Shader is identical to v77's GLSL3 RawShaderMaterial (warped fbm + cnoise +
// 3 wave layers, linear-burn blends through low/main/mid/high bloop colors).

const { useEffect: useEArt, useRef: useRArt, useMemo: useMArt } = React;

/* ---- palettes (from src/data/artsky-extended-palettes.ts) ---- */
const ARTSKY_PALETTES = {
  cerulean:  ["#050a10","#0a1622","#122334","#1b3448","#38bdf8","#e0f2fe"], // homepage
...
function hexToVec3(hex){ const h=hex.slice(1); return [parseInt(h.slice(0,2),16)/255, parseInt(h.slice(2,4),16)/255, parseInt(h.slice(4,6),16)/255]; }
function avgVec3(a,b){ return [(a[0]+b[0])/2,(a[1]+b[1])/2,(a[2]+b[2])/2]; }
...
/* ---- shaders (verbatim from v77 artsky-core.ts) ---- */
...
/* ---- minimal helpers (no Three.js dep) ---- */
...
function ArtSky({
  ...
  const hostRef = useRArt(null);
  const canvasRef = useRArt(null);
  const stateRef = useRArt({});

  const colors = useMArt(() => paletteToBloop(ARTSKY_PALETTES[theme] || ARTSKY_PALETTES.cerulean), [theme]);
  ...
  useEArt(() => {
    ...
    let gl = canvas.getContext("webgl2", { antialias: false, alpha: false, premultipliedAlpha: false, powerPreference: "high-performance" });
    if (!gl) {
      // fallback: leave a CSS fallback in the host
      host.dataset.gl = "no";
      return;
    }
    let vs, fs, prog;
    ...
    // RAF loop
    let raf, last = performance.now(), tAcc = 0, alive = true;
    const tick = (now) => {
      if (!alive) return;
      ...
      // colors / params come from latest render (refresh via stateRef)
      const c = s.colors;
      if (c) {
  ...
  // Push live params into ref every render
  ...
  return (
    <div ref={hostRef} aria-hidden="true" style={{
...
Object.assign(window, { ArtSky, ARTSKY_PALETTES });
